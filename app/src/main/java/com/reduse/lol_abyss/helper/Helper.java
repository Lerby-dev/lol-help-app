package com.reduse.lol_abyss.helper;
...
public class Helper {
    ...
    public static void setImageItems(Context context, int item, ImageView image){
        ...
        if(item != 0){
            ...
            Picasso.with(context).load("http://ddragon.leagueoflegends.com/cdn/11.8.1/img/item/"+item+".png ").error(R.drawable.empty).into(image);
    ...
    public static void setImagePortraits(Context context, String champId, ImageView image, ApiRequest request){

        String portrait = null;

        try {
            portrait = request.getChampionName(champId);
            Picasso.with(context).load("http://ddragon.leagueoflegends.com/cdn/11.8.1/img/champion/"+portrait).into(image, new Callback() {
                @Override
